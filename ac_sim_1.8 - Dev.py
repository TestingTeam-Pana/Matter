...
def update_ac_ui_status():
    ...
    if g_lt_status == 0x00 :
        lt_status_str = f"Success ({g_lt_status})"
    elif g_lt_status > 0x00 :     
        lt_status_str = f"Failure ({g_lt_status})"
    else:
        lt_status_str = f""           

    psn_str = ''.join(chr(i) for i in g_hamcu_psn)
    mdl_str = ''.join(map(str, g_model_id))
    ...
    mdl_status.config(text=f"{mdl_str}")
    lt_status.config(text=f"{lt_status_str}")
...
entry4_label = tk.Label(root, text="LineTest PSN (Non-Matter)")
entry5_label = tk.Label(root, text="LineTest QRCode (Matter)")
...
mdl_label = tk.Label(root, text="Modelid")
mdl_label.grid(row=status_row+5, column=2, sticky="w", padx=5, pady=5)
mdl_status = tk.Label(root, text="  " , background = txt_bg_color)
mdl_status.grid(row=status_row+5, column=3, sticky="w", padx=5, pady=5)

lt_status_label = tk.Label(root, text="LineTest Status")
lt_status_label.grid(row=status_row+6, column=0, sticky="w", padx=5, pady=5)
lt_status = tk.Label(root, text="  " , background = txt_bg_color)
lt_status.grid(row=status_row+6, column=1, sticky="w", padx=5, pady=5)
